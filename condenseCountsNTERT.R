...
my_countFiles <- c("cont1.counts", "cont2.counts", "cont3.counts", "UV1.counts", "UV2.counts", "UV3.counts")
...
norm_UV <- data.frame(gene_names=unique_gene_names, UV1_charged=normalized_isodecoder_counts[[4]]$Charged_Count, UV2_charged=normalized_isodecoder_counts[[5]]$Charged_Count, UV3_charged=normalized_isodecoder_counts[[6]]$Charged_Count,
                      UV1_uncharged=normalized_isodecoder_counts[[4]]$Uncharged_Count, UV2_uncharged=normalized_isodecoder_counts[[5]]$Uncharged_Count, UV3_uncharged=normalized_isodecoder_counts[[6]]$Uncharged_Count,
                      UV_charged_avg=rep(0,52), UV_charged_SD=rep(0,52), UV_uncharged_avg=rep(0,52), UV_uncharged_SD=rep(0,52), UV_chargePercent_avg=rep(0,52), UV_chargePercent_sd=rep(0,52), UV_totalSD=rep(0,52))
norm_avg[["UV"]] <- norm_UV
...
norm_avg2[["UV"]] <- norm_avg[["UV"]][-c(42,52),]
...
norm_avg3[["UV"]] <- norm_avg2[["UV"]][-c(39,41), ]

#prepare data for heatmap
library(ggplot2)

#plot control and UV only
...
heatmap <- ggplot(data, aes(x=treatment,y=factor(gene_name, levels = rev(levels(factor(gene_name)))),fill = charge_percent))+geom_tile() + scale_fill_gradient(low = "red", high = "blue") +
     ...
     theme(axis.text.x = element_text(angle = 45, vjust = 1.0, hjust = 1.0)) + coord_equal() + scale_x_discrete(labels = c("Control", "UV"))
#ggsave("HeatmapIsodecoderUV.png", width = 1200, height = 2400, units = "px", dpi = 300)
...
#ggsave("IsodecoderChargingUV.png", plot = my_charge_barplot, width = 2400, height = 1200, units = "px", dpi = 300)
...
for (j in 1:length(norm_avg3[[1]]$gene_names)){
     UV_pvalues <- c(UV_pvalues, t.test.Welch(norm_avg3[[1]]$cont_chargePercent_avg[j], norm_avg3[[2]]$UV_chargePercent_avg[j], norm_avg3[[1]]$cont_chargePercent_sd[j], norm_avg3[[2]]$UV_chargePercent_sd[j], 3, 3)[5])
}
cont_pvalues <- rep(1,48)
data$pvalues <- as.vector(rbind(cont_pvalues, UV_pvalues))
adj_pvalues <- c()
...
#UV 
UV_stats <- data.frame(gene_names=norm_avg3[["UV"]]$gene_names, chargePercent=norm_avg3[["UV"]]$UV_chargePercent_avg, p_value=UV_pvalues)
UV_stats_tbl <- as_tibble(UV_stats)
UV_stats_tbl_sorted <- arrange(UV_stats_tbl, p_value)
UV_stats_tbl_sorted$rank <- seq(1:48)
UV_stats_tbl_sorted$BUVDR <- UV_stats_tbl_sorted$p_value*48/UV_stats_tbl_sorted$rank
...
colnames(aa_stats) <- c("amino_acid", "control_avg", "control_sd", "UV_avg", "UV_sd","UV_p.value")
...
for (i in 1:length(codon_table)){
     ...
     UV_vect <- norm_avg3[[2]][grep(pattern = names(codon_table)[i], x = norm_avg3[[1]]$gene_names, ignore.case = TRUE), 12]
     

     control_mean <- sum(control_vect)/length(control_vect)
     UV_mean <- sum(UV_vect)/length(UV_vect)
     
     
     #calculate SD
     control_vectsd <- norm_avg3[[1]][grep(pattern = names(codon_table)[i], x = norm_avg3[[1]]$gene_names, ignore.case = TRUE), 13]
     UV_vectsd <- norm_avg3[[2]][grep(pattern = names(codon_table)[i], x = norm_avg3[[1]]$gene_names, ignore.case = TRUE), 13]
    
     control_sd <- sqrt(sum(control_vectsd^2))/length(control_vectsd)
     UV_sd <- sqrt(sum(UV_vectsd^2))/length(UV_vectsd)
  
     
     #add values to data frame
     aa_stats[i, 2] <- control_mean
     aa_stats[i, 3] <- control_sd
     aa_stats[i, 4] <- UV_mean
     aa_stats[i, 5] <- UV_sd
  
     pval_temp <- t.test.Welch(m1=control_mean, m2=UV_mean, s1=control_sd, s2=UV_sd, 3,3)
...
UV_aa_stats_tbl_sorted <- arrange(aa_stats_tbl, UV_p.value)
UV_aa_stats_tbl_sorted$UV_rank <- seq(1:20)
UV_aa_stats_tbl_sorted$UV_BUVDR <- UV_aa_stats_tbl_sorted$UV_p.value*20/UV_aa_stats_tbl_sorted$UV_rank
...
x = c("Control", "UV")
data = expand.grid(X=x,Y=y)
data$charge_percent <- as.vector(rbind(aa_stats_tbl$control_avg, aa_stats_tbl$UV_avg))
data$charge_percentSD <- as.vector(rbind(aa_stats_tbl$control_sd, aa_stats_tbl$UV_sd))
...
heatmap <- ggplot(data, aes(x=treatment,y=factor(amino_acid, levels = rev(levels(factor(amino_acid)))),fill = charge_percent))+geom_tile() + scale_fill_gradient(low = "red", high = "blue") +
     ...
     theme(axis.text.x = element_text(angle = 45, vjust = 1.0, hjust = 1.0)) + coord_equal() + scale_x_discrete(labels = c("Control", "UV"))
#ggsave("HeatmapAminoAcidUV.png", width = 1200, height = 2400, units = "px", dpi = 300)
...
#ggsave("AminoAcidChargingUV.png", plot = my_charge_barplot, width = 2400, height = 1200, units = "px", dpi = 300)
